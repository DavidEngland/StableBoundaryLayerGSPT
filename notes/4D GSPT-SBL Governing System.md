...
$$\varepsilon \frac{de}{dt} =
 \sqrt{e + \delta} \left[ \eta\,\gamma\,(U^2 + V^2) - K \cdot G(T_s) \right] - \frac{(e+\delta)^{3/2}}{l_0} = \mathcal{F}(e,U,V,T_s)$$

Slow Subsystem (Atmospheric Rotational Dynamics & Surface Thermodynamics)

$$\frac{dU}{dt} = f(V - V_g) - \gamma \sqrt{e + \delta} \cdot U$$
$$\frac{dV}{dt} = -f(U - U_g) - \gamma \sqrt{e + \delta} \cdot V$$
$$\frac{dT_s}{dt} = \frac{1}{C_{\text{skin}}} \left[ R_{\downarrow} - \sigma_{SB} T_s^4 - \lambda \frac{T_s - T_{\text{deep}}}{d_{\text{soil}}} - \rho c_p C_H \sqrt{e + \delta} \cdot (T_s - T_a) \right]$$
...
| $f$ | Coriolis Parameter $(\sim 45^\circ\text{N})$ | $1.0 \times 10^{-4}$ | $\text{s}^{-1}$ |

Turbulence and Boundary Layer Internals
