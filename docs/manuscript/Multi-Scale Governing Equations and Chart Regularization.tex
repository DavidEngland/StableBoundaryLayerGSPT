...
\end{enumerate}
Unless otherwise stated, we use $\Omega_0$ to denote the full mathematical chart domain and $\Omega_{\mathrm{phys}}$ to denote the positively invariant physical region used for geometric analysis.
We further assume that $\Omega_0$ extends across $\partial\Omega_{\mathrm{phys}}$ to include a neighborhood of $\tilde e = 0$, so that $\mathcal S_0$ and $\mathcal C_{\mathrm{fold}}$ can be analyzed as smooth embedded submanifolds near the laminar boundary.
...
= \epsilon_1\tilde e \cdot \frac{1}{2\tilde e}\frac{1}{\epsilon_1}\frac{de}{dt}
= \frac{1}{2}\frac{de}{dt}.
\end{equation}
...
= \begin{pmatrix}
\tilde e\left[\mathcal{F}_{\mathrm{ls}} - \frac{\partial}{\partial z}\big(c_m \ell \tilde e S\big)\right] \\
\frac{\tilde e}{C_s}\left[R_{\mathrm{net}}(T_s) + \rho c_p q_\theta + \frac{k_g}{d_g}(T_g - T_s)\right]
\end{pmatrix},
\end{equation}
where the factors $\tilde e$ arise from the positive time reparameterization, so the slow tendencies freeze in fast time as $\tilde e \to 0^+$.
The corresponding net radiation closure is
\begin{equation}
R_{\mathrm{net}}(T_s)
...
= \tilde e\,\frac{k_g}{d_g^2}(T_s - T_g).
\end{equation}
As in the slow subsystem, the prefactor $\tilde e$ is induced by the positive time change; all geometric arguments in subsequent sections are restricted to $\Omega_{\mathrm{phys}}$.
...
The differential covector of the Richardson functional is
\begin{equation}
D\pi_{Ri}(\mathbf{x})
= \begin{pmatrix}
0 & 0 & -\dfrac{2g\,\theta_z(T_s)}{\theta_0 S^3} & \dfrac{g}{\theta_0 S^2}\dfrac{\partial \theta_z}{\partial T_s} & 0
\end{pmatrix}.
\end{equation}
