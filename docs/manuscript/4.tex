...
In this section, we apply Geometric Singular Perturbation Theory (GSPT) to resolve this paradox. We establish that observed scalar Richardson thresholds are not universal physical constants, but rather site-specific projections of an invariant fold locus $\mathcal{C}_{\mathrm{fold}} \subset \Omega_0$ onto diagnostic observation spaces.

\subsection{Diagnostic Observation Mapping and Differential Geometry}
...
To understand how higher-dimensional state space trajectories project onto diagnostic measurements, we analyze the differential geometric properties of $\boldsymbol{\Pi}_{\mathrm{obs}}$ restricted to the turbulent critical manifold branch $\mathcal{S}_0^+$.
...
\end{align}
Since $\tilde e > 0$, $S^* > 0$, $A(T_s) > 0$, $B > 0$, and assuming non-vanishing surface thermal gradients ($\theta_z'(T_s) \neq 0$), each factor in \eqref{eq:jac_det_explicit} is strictly non-zero. Thus $\det(\mathbf{J}_{\mu}) \neq 0$, proving that $\operatorname{rank}\left(D\left(\boldsymbol{\Pi}_{\mathrm{obs}}\vert_{\mathcal{S}_0^+}\right)\right) = 2$ everywhere on $\mathcal{S}_0^+$.
\end{proof}

\begin{corollary}[Dimension and Immersion of the Fold Image]
\label{cor:fold_image}
The fold locus $\mathcal{C}_{\mathrm{fold}} = \{\mathbf{x} \in \mathcal{S}_0^+ \mid \det J_{\mathrm{fast}}(\mathbf{x}) = 0\}$ is a 2-dimensional smooth embedded submanifold of $\Omega_0 \subset \mathbb{R}^5$ parameterized by $(T_s, T_g)$. Under diagnostic observation $\boldsymbol{\Pi}_{\mathrm{obs}}$, the deep soil temperature $T_g$ generates a 1-dimensional fiber along which measurements are invariant. Consequently, the fold image $\Gamma_{\mathrm{fold}} \equiv \boldsymbol{\Pi}_{\mathrm{obs}}(\mathcal{C}_{\mathrm{fold}}) \subset \mathcal{O}$ is a 1-dimensional smooth immersed submanifold (space curve) in diagnostic space parameterized smoothly by $T_s$. Assuming strict monotonicity of the thermal gradient mapping $T_s \mapsto \theta_z(T_s)$ over the physical domain, $\Gamma_{\mathrm{fold}}$ is a smooth embedded curve in $\mathcal{O}$.
\end{corollary}

\begin{proof}
By Fenichel theory and the regular value theorem, $\dim(\mathcal{C}_{\mathrm{fold}}) = 3 - 1 = 2$. On $\mathcal{C}_{\mathrm{fold}}$, fast variables are fixed functions of $T_s$: $\tilde e_{\mathrm{fold}}(T_s) = \frac{g c_w \ell^2}{2\theta_0 C_\theta}\theta_z(T_s)$ and $S_{\mathrm{fold}}(T_s) = \frac{g c_w \ell}{2\theta_0 C_\theta \sqrt{c_m}}\theta_z(T_s)$. Because $\boldsymbol{\Pi}_{\mathrm{obs}}$ has no explicit dependence on $T_g$, the $T_g$-coordinates collapse under projection. The remaining reduced map $T_s \mapsto \boldsymbol{\Pi}_{\mathrm{obs}}(\mathcal{C}_{\mathrm{fold}})$ has differential rank 1 whenever $\theta_z'(T_s) \neq 0$. If $T_s \mapsto \theta_z(T_s)$ is strictly monotonic, this parameterization is injective, rendering $\Gamma_{\mathrm{fold}}$ an embedded 1D space curve in $\mathbb{R}^3$.
\end{proof}
...
\mathcal{O} \subset \mathbb{R}^3 & \xleftarrow{\quad \text{Site Constraints} \quad} & \Gamma_{\mathrm{fold}} \subset \mathbb{R}^3 & \xleftarrow{\quad T_g \text{-Fiber Collapse} \quad} & \boldsymbol{\Pi}_{\mathrm{obs}}(\mathcal{C}_{\mathrm{fold}}) \\[4pt]
\text{($Ri_{\mathrm{obs}}$ Intersects)} & & \text{(1D Fold Curve)} & &
\end{matrix}
\end{equation}
...
\begin{tikzpicture}[x=1cm, y=1cm, >=Stealth]
    ...
    % Fold Locus C_fold (2D surface)
    \draw[red, ultra thick] (0.3,2.2) to[out=15,in=195] (4.9,3.0);
    \node[red!90!black, font=\footnotesize\bfseries, anchor=south] at (2.6,2.6) {Fold Locus $\mathcal{C}_{\mathrm{fold}}$ (2D Surface)};
    ...
    % Fold Curve Image Gamma_fold (1D space curve in diagnostic space)
    \draw[red, ultra thick] (8.4,4.3) to[out=-60,in=170] (12.5,0.9);
    \node[red!90!black, font=\small\bfseries] at (11.0,2.1) {$\Gamma_{\mathrm{fold}} = \boldsymbol{\Pi}_{\mathrm{obs}}(\mathcal{C}_{\mathrm{fold}})$ (1D Curve)};
...
We evaluate the dimensionless fold formula \eqref{eq:Ri_fold_nondim} across two contrasting campaign regimes using calibrated turbulence closure parameters ($c_m = 0.06$, $c_w = 0.35$, $C_\theta = 0.20$, yielding dimensionless group $\mathcal{A}_{\mathrm{turb}} \equiv \frac{4 C_\theta^2 c_m}{c_w^2} = 4.20$) and standard boundary-layer scales ($\ell = 10\,\text{m}$, $u_0 = 0.10\,\text{m/s}$):
...
\begin{enumerate}
    ...
    \begin{equation}
    Ri_{\mathrm{fold}}^{\mathrm{CASES}} \approx 0.18 - 0.23.
    \end{equation}
    ...
    \begin{equation}
    Ri_{\mathrm{fold}}^{\mathrm{SHEBA}} \approx 0.70 - 1.20.
    \end{equation}
...
\begin{table}[h]
\centering
\caption{\textbf{Comparison of Campaign Observational Profiles and GSPT Predictions.} Dimensionless stratification scale $\Theta_z \equiv \frac{g}{\theta_0}\theta_z \frac{\ell^2}{u_0^2}$ is computed from published campaign mean inversions $\theta_z$ using boundary-layer scales $\ell = 10\,\text{m}$ and $u_0 = 0.10\,\text{m/s}$. Numerical values for $Ri_{\mathrm{fold}}$ are evaluated using the closure group $\mathcal{A}_{\mathrm{turb}} = 4.20$.}
