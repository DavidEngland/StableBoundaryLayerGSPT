...
function getnested(obj, keys::Vector{String}, default="n/a")
    ...
    for k in keys
        if cur isa AbstractDict && haskey(cur, k)
            cur = cur[k]
        elseif cur isa AbstractDict && haskey(cur, Symbol(k))
            cur = cur[Symbol(k)]
        elseif cur isa JSON3.Object && haskey(cur, Symbol(k))
            cur = cur[Symbol(k)]
        elseif cur isa JSON3.Object && haskey(cur, k)
            cur = cur[k]
        else
            return default
        end
...
function main()
    ...
    # 1. Always build active and campaign parameter context and write parameter macro bundle
    context, macro_path, active_params = write_parameter_macro_bundle(dataset)

    # 2. Check parameter drift if requested
    if check_parameter_drift
        verify_parameter_macro_bundle!(macro_path, dataset, active_params)
        println("[check-parameter-drift] verified parameter macro bundle consistency at $(macro_path)")
    end

    # 3. Lint prose literals if requested
    if lint_prose_literals
        lint_prose_literals!(dataset; strict=lint_prose_strict, allowlist_path=lint_prose_allowlist_path)
    end

    # 4. Exit early if only asked to write parameter macros or check parameter drift
    if write_parameter_macros_only
        println("[assemble-manuscript] wrote parameter macro bundle to $(macro_path)")
        return
    end

    # 5. Full manuscript compilation
    tex_output_dir = joinpath("reports", "generated")
    mkpath(tex_output_dir)

    # Inject additional dynamic context key-values
    context["generated_date_human"] = generated_date_human
    context["generated_figures_tex"] = build_tex_figure_includes(joinpath("reports", "generated", "figures"); tex_output_dir=tex_output_dir)
    context["sections_tex"] = build_tex_template_sections(joinpath("templates", "sections"), context)
    context["abstract_tex"] = build_optional_tex_template(joinpath("templates", "sections", "abstract.tex.mustache"), context)

    # Primary paper template location check
    paper_template_path = joinpath("templates", "paper.tex.mustache")
    if !isfile(paper_template_path)
        paper_template_path = joinpath("templates", "paper.tex")
    end

    if isfile(paper_template_path)
        paper_text = read_text(paper_template_path)
        rendered_paper = render_template(paper_text, context)
        paper_out_path = joinpath(tex_output_dir, "paper.tex")
        write(paper_out_path, rendered_paper)
        println("[assemble-manuscript] assembled manuscript paper TeX -> $(paper_out_path)")
    else
        @warn "No paper template found at templates/paper.tex.mustache or templates/paper.tex"
    end
end
